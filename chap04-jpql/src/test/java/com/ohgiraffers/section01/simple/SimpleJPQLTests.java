...
public class SimpleJPQLTests {
    ...
    * */
    ...
    /*
    * JPQL 사용방법은 다음과 같다.
    * 1. 작성한 jqpl(문장열)을 "EntityManager.createQuery()" 메소드를 통해 쿼리 객체로 만든다.
    * 2. 쿼리 객체는 "TypedQuery", "Query" 두 가지가 있다.
    * - TypedQuery : 반환할 타입을 명확하게 지정하는 방식일 때 사용하며 쿼리 객체의 메소드 실행 결과로 지정한 타입이 반환된다.
    * - Query : 반환할 타입을 명확하게 지정할 수 없을 때 사용하며 쿼리 객체 메소드의 실행 결과로 Object 또는 Object[]가 반환 된다.
    * 3. 쿼리 객체에서 제공하는 메소드 "getSingleResult()" 또는 "getResultList()"를 호출해서 쿼리를 실행하고 데이터에비스를 조회한다.
